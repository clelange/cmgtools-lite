...
class DataCardMaker:
    ...
    def addMVVSignalParametricShape(self, name, variable, jsonFile, scale={}, resolution={}):
        ...
        vvMass = ROOT.RooDoubleCB(pdfName, pdfName, self.w.var(MVV), self.w.function(SCALEVar), self.w.function(
            SIGMAVar), self.w.function(ALPHA1Var), self.w.function(N1Var), self.w.function(ALPHA2Var), self.w.function(N2Var))
    ...
    def addMJJSignalParametricShape(self, name, variable, jsonFile, scale={}, resolution={}, varToReplace="MH"):
        ...
        SIGMAVar = "_".join(["sigma", name, self.tag])
        self.w.factory("expr::{name}('({param})*(1+{vv_syst})',MH,{vv_systs})".format(name=SIGMAVar,
                                                                                      param=info['sigma'], vv_syst=resolutionStr, vv_systs=','.join(resolutionSysts)).replace("MH", varToReplace))
    ...
    def addMJJSignalParametricShapeCB(self, name, variable, jsonFile, scale={}, resolution={}, varToReplace="MH"):
        ...
        pdfName = "_".join([name, self.tag])
        vvMass = ROOT.RooDoubleCB(pdfName, pdfName, self.w.var(MJJ), self.w.function(SCALEVar), self.w.function(
            SIGMAVar), self.w.function(ALPHA1Var), self.w.function(N1Var), self.w.function(ALPHA2Var), self.w.function(N2Var))
        getattr(self.w, 'import')(vvMass, ROOT.RooCmdArg())
        f.close()
    ...
    def addMJJTopShapeCB(self, name, variable, jsonFile, scale={}, resolution={}, varToReplace="MH", uncertainties=[0, 0, 0, 0], newTag=""):
        ...
        vvMass = ROOT.RooDoubleCB(pdfName1, pdfName1, self.w.var(MJJ), self.w.function(SCALEVar), self.w.function(
            SIGMAVar), self.w.function(ALPHAVar1), self.w.function(NVar1), self.w.function(ALPHAVar2), self.w.function(NVar2))
        getattr(self.w, 'import')(vvMass, ROOT.RooCmdArg())
    ...
    def addMVVBackgroundShapeQCD(self, name, variable, logTerm=False, newTag="", preconstraints={}):
        ...
        p0 = "_".join(["p0", tag])
        if "p0" in preconstraints.keys():
            val = preconstraints['p0']['val']
            err = preconstraints['p0']['err']
        ...
        p1 = "_".join(["p1", tag])
        if "p1" in preconstraints.keys():
            val = preconstraints['p1']['val']
            err = preconstraints['p1']['err']
        ...
        p2 = "_".join(["p2", tag])
        if "p2" in preconstraints.keys():
            val = preconstraints['p2']['val']
            err = preconstraints['p2']['err']
    ...
    def addMVVBackgroundShapeQCDJJStyle(self, name, variable, degree, newTag="", preconstraints={}):
        ...
        ranges["p2"] = [0, 1000]
        defaultValues = {}
        defaultValues["p0"] = 15.0
        defaultValues["p1"] = 0.001
        defaultValues["p2"] = 0.001
        parNames = []
        ...
        if (degree == 3):
            parName = "_".join(["p2", tag])
            self.w.factory("{name}[0]".format(name=parName))
            parNames.append(parName)

        pdfName = "_".join([name, self.tag])
        qcd = None
        if (degree == 2):
            qcd = ROOT.RooPowPdf(pdfName, pdfName, self.w.var(MVV), self.w.var(parNames[0]))
        else:
            qcd = ROOT.RooQCDPdf(pdfName, pdfName, self.w.var(MVV), self.w.var(parNames[0]), self.w.var(parNames[1]), self.w.var(parNames[2]))
        getattr(self.w, 'import')(qcd, ROOT.RooCmdArg())

    def addMVVBackgroundShapePow(self, name, variable, newTag="", preconstraints={}):
    ...
    def addMVVBackgroundShapeErfPow(self, name, variable, newTag="", preconstraints={}):
