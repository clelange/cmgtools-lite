...
# import copy
import array
from CMGTools.VVResonances.plotting.CMS_lumi import cmslabel_int
...
def performFit(hMass, fNbins, fBins, fFitXmin, fFitXmax, fLabel,  fOutputFile, doSigmaBand):

    fits = []
    residuals = []

    xbins = array.array('d', fBins)

    hMass.SetBinErrorOption(ROOT.TH1.kPoisson)
    ...
    hMassNEW = ROOT.TH1F("hist_mass", "", fNbins, xbins)
...
# ---------------------------- Create graph from data with Poisson error bars (for data) ----------------------------
    ...
    eyh = []
    alpha = 1 - 0.6827
    hMass_rebinned.SetBinErrorOption(ROOT.TH1.kPoisson)
    for i in range(0, fNbins):
        ...
        if(n == 0):
            ...
            l = ROOT.Math.gamma_quantile(alpha / 2, n, 1.)  # as recommended in https://twiki.cern.ch/twiki/bin/view/CMS/PoissonErrorBars
        h = ROOT.Math.gamma_quantile_c(alpha / 2, n + 1, 1)
        eyl.append((n - l) / (dm))
        eyh.append((h - n) / (dm))
        ...
        print "dm %f" % dm
        print "n/dm %f" % (n / dm)
        print " sqrt(n)/dm : %f" % (ROOT.TMath.Sqrt(n) / dm)
        print " sqrt(n/dm) : %f" % ROOT.TMath.Sqrt(n / dm)

    vx = array.array("f", x)
    vy = array.array("f", y)
    vexl = array.array("f", exl)
    vexh = array.array("f", exh)
    veyl = array.array("f", eyl)
    veyh = array.array("f", eyh)

    # data in TGraph format
    g = ROOT.TGraphAsymmErrors(fNbins, vx, vy, vexl, vexh, veyl, veyh)
    g.SetName("g_data")
...
#     print  " You should check for higher-order polynomials. "
    ...
    print "\hline"
    print "\end{tabular}"
    boson, purity, lepton = fLabel.split("_", 3)
    print "\caption{Residuals, $\chi^{2}$, and degrees of freedom for %s %s %s category. A %i parameter fit is needed to describe these data.}" % (boson.strip("_"), purity.strip("_"), lepton.strip("_"), prmt)
    print "\label{tab:%s}" % fLabel
    print "\end{table}"
    with open("%s/f-test_VV.tex" % outdir, "a") as text_file:
        ...
        boson, purity, lepton = fLabel.split("_", 3)
        text_file.write("\caption{Residuals, $\chi^{2}$, and degrees of freedom for %s %s %s category. A %i parameter fit is needed to describe these data.}\n" % (boson.strip("_"), purity.strip("_"), lepton.strip("_"), prmt))
...
def doFit(FunctionType, hMassNEW, graph, fFitXmin, fFitXmax, fNbins, xbins, fLabel):
    print ""
    print ""
    print ""
    print ""
    ...
    if(FunctionType == -2):
        nPar = 2
        BKGfit = ROOT.TF1("BKGfit%i" % FunctionType, " [0] / ( TMath::Power(x/13000,[1]) )", fFitXmin, fFitXmax)
    ...
    if(FunctionType == 0):
        ...
        nPar = 3
        BKGfit = ROOT.TF1("BKGfit%i" % FunctionType, "( [0]*TMath::Power(1-x/13000,[1]) ) / ( TMath::Power(x/13000,[2]) )", fFitXmin, fFitXmax)
        ...
        nPar = 4
        BKGfit = ROOT.TF1("BKGfit%i" % FunctionType, "( [0]*TMath::Power(1-x/13000,[1]) ) / ( TMath::Power(x/13000,[2]+[3]*log(x/13000)) )", fFitXmin, fFitXmax)
        ...
        nPar = 4
        BKGfit = ROOT.TF1("BKGfit%i" % FunctionType, "( [0]*TMath::Power(1-x/13000 + [3]*TMath::Power(x/13000,2),[1]) ) / ( TMath::Power(x/13000,[2]) )", fFitXmin, fFitXmax)
    ...
    for loop in range(0, 10):
        ...
        if(fitStatus == 0):
            ...
            # r.Print("V")
            break
    c1 = ROOT.TCanvas("c3", "fit", 600, 400)
    hMassNEW.Draw()
    # suffix = "WHP"
    # if fLabel.find('W') != -1:
    #     suffix = "W"
    ...
    # if fLabel.find('LP') != -1:
    #     suffix += "LP"
    c1.SaveAs(outdir + "/test_" + fLabel + ".pdf")
    # if(stopProgram==1):
    # print "######################"
    # print"######################"
    ...
    histoCI = ROOT.TGraphErrors(fNbins)
    histoCI.SetTitle("Fitted line with .95 conf. band")
    for i in range(0, fNbins):
        histoCI.SetPoint(i, graph.GetX()[i], 0)
    ...
    hist_fit_residual_vsMass = ROOT.TH1D("hist_fit_residual_vsMass", "hist_fit_residual_vsMass", fNbins, xbins)

    for bin in range(1, hMassNEW.GetNbinsX()+1):
        # if(hMassNEW.GetXaxis().GetBinLowEdge(bin) >= fFitXmin and hMassNEW.GetXaxis().GetBinUpEdge(bin) <= fFitXmax):
        if(hMassNEW.GetXaxis().GetBinLowEdge(bin) >= fFitXmin and hMassNEW.GetXaxis().GetBinCenter(bin) <= fFitXmax):
            NumberOfVarBins += 1
            data = hMassNEW.GetBinContent(bin)
            # data = g.Integral(hMassNEW.GetXaxis().GetBinLowEdge(bin) , hMassNEW.GetXaxis().GetBinUpEdge(bin) )
            err_data_low = graph.GetErrorYlow(bin - 1)
            err_data_high = graph.GetErrorYhigh(bin - 1)
...
def FisherTest(RSS_1, RSS_2, dof_1, dof_2, N):
    ...
    F_dist = ROOT.TF1("F_distr", "TMath::Sqrt( (TMath::Power([0]*x,[0]) * TMath::Power([1],[1])) / (TMath::Power([0]*x + [1],[0]+[1])) ) / (x*TMath::Beta([0]/2,[1]/2))", 0, 1000)
    F_dist.SetParameter(0, n2 - n1)
    F_dist.SetParameter(1, N - n2)
    CL = 1 - ROOT.F_distr.Integral(0.00000001, F)
    alternateCL = 1. - ROOT.TMath.FDistI(F, n2 - n1, N - n2)
    return [F, CL, alternateCL, F_dist]


def DrawFit(hMassNEW, graph, M1Bkg, hist_fit_residual_vsMass, FunctionType, nPar, fFitXmin, fFitXmax, fLabel, fOutputFile):
    W = 600
    H = 700
    # H_ref = 700
    # W_ref = 600
    ...
    c1 = ROOT.TCanvas("c1", "VV mass fit", W, H)
    c1.GetWindowHeight()
    c1.GetWindowWidth()
    c1.SetLogy()
    c1.Divide(1, 2, 0, 0, 0)
    ...
    addInfo = ROOT.TPaveText(0.2358691, 0.04035043, 0.5050171, 0.1870085, "NDC")
    ...
    vFrame.SetTitle("")
    vFrame.SetXTitle("M_{l#nuJ} (GeV)")
    vFrame.SetYTitle("#frac{dN}{dm_{l#nuJ}} (GeV^{-1})")
    ...
    graph.GetXaxis().SetNdivisions(405)
    graph.Draw("pe0 same")
    M1Bkg[0].SetLineWidth(2)
    M1Bkg[0].SetLineStyle(1)
    M1Bkg[0].SetLineColor(ROOT.kRed)
    M1Bkg[0].Draw("same")

    legend = ROOT.TLegend(0.56164991, 0.62, 0.8503575, 0.80)
    ...
    hist_fit_residual_vsMass[0].SetLineColor(ROOT.kBlack)
    hist_fit_residual_vsMass[0].SetMarkerColor(ROOT.kBlack)
    ...
    line = ROOT.TLine(fFitXmin, 0, fFitXmax, 0)
    line.Draw("same")
    p11_2.RedrawAxis()
    line2 = ROOT.TLine()
...
def FitComparisons(hMassNEW, graph, M1Bkg, hist_fit_residual_vsMass, FunctionType, nPar, fFitXmin, fFitXmax, fLabel, fOutputFile, chi2, dof, doSigmaBand, histoCI):
    ...
    c2 = ROOT.TCanvas("c2", "c2", 50, 50, W, H)
    c2.SetFillColor(0)
    c2.SetBorderMode(0)
    ...
    # addInfo = ROOT.TPaveText(0.1397805, 0.01676406, 0.4371859, 0.2227225, "NDC")
    addInfo = ROOT.TPaveText(0.1397805, 0.01676406, 0.4371859, 0.0827225, "NDC")
    addInfo.AddText(fLabel.replace("_", " ").replace("mu", "#mu"))
    ...
    vFrame.SetTitle("")
    vFrame.SetXTitle("Resonance invariant mass (GeV)")
    vFrame.SetYTitle("#frac{dN}{dm_{l#nuJ}} (GeV^{-1})")
    # vFrame.GetXaxis().SetTitleSize(0.06)
#   vFrame.GetXaxis().SetTitleOffset(0.95)
#   vFrame.GetXaxis().SetLabelSize(0.05)
...
#   vFrame.GetYaxis().SetLabelSize(0.05)
    ...
    legend = ROOT.TLegend(0.5512768, 0.4885525, 0.7217721, 0.907654)
    if (doSigmaBand):
        legend = ROOT.TLegend(0.5512768, 0.4885525, 0.7217721, 0.907654)
    ...
    legend.SetFillStyle(0)
    legend.SetMargin(0.35)
    legend.AddEntry(graph, legend_text, "lpe")
    if (doSigmaBand):
...
#   legend.AddEntry(M1Bkg[3], "5 par.   (#chi^{2}/ndof = %.2f/%i)"%(chi2[3],dof[3]),"l")
    ...
    vFrame2.SetTitle("")
    vFrame2.SetXTitle("Resonance invariant mass (GeV)")
    vFrame2.GetXaxis().SetTitleSize(0.06)
    vFrame2.SetYTitle("#frac{Data-Fit}{#sigma}")
...
def getHisto(sampleTypes, cuts, catName, resonanceMassString, templateDir, logTerm=False):
    # minMVV = 1000
    # maxMVV = 5000
    ...
    histo = data.drawTH1(resonanceMassString, cuts, "1", maxMVV-minMVV, minMVV, maxMVV)
    return histo
        ...
        for purity in purities:
            for boson in bosons:
                for categ in b_categories:
                    ...
                    logTerm = False
                    histo = getHisto(dataTemplate, cuts, catName, resonanceMassString, templateDir, logTerm)
                    # performFit(histo, len(massBins) - 1, massBins, minMVV, fitmax, "%s category, %s, %s channel" % (boson, purity, lepton), "%s/%s" % (outdir, catName), doSigmaBand=False)
                    performFit(histo, len(massBins) - 1, massBins, minMVV, fitmax, "%s_%s_%s" % (boson, purity, lepton), "%s/%s" % (outdir, catName), doSigmaBand=False)
