...
plotter.addContribution("QCD", False, " QCD", 2, 1, ROOT.kBlack, 1001, ROOT.kSpring - 5)
    ...
    for purity in purities:
        for boson in bosons:
            for categ in b_categories:
                ...
                plotter.drawBinned("MVV", "m_{WV} (GeV)", catName, [], 0)  # from ipython
                cmslabel_prelim(plotter.pad1, '2016', 11)
                plotter.canvas.SaveAs("postFitMJJ_" + catName + ".root")
                plotter.canvas.SaveAs("postFitMJJ_" + catName + ".pdf")
                plotter.canvas.SaveAs("postFitMJJ_" + catName + ".png")
